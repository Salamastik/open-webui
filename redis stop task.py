...
# Flag לעצירת משימה
REDIS_TASK_STOP_KEY_FMT = f"{REDIS_KEY_PREFIX}:tasks:stop:{{}}"
STOP_KEY_FMT = "task:stop:{}"
STOP_KEY_TTL_SECONDS = 3600  # זמן חיים לדגל העצירה


async def redis_task_command_listener(app):
    """
    Listener that tries to receive pubsub notifications (best-effort).
    On receiving a notification it checks the stop-flag and cancels local task if needed.
    Note: זה רק מזעיק בדיקה מהירה — ה-flag הוא ה־source-of-truth הממשי.
    """
    ...
    async for message in pubsub.listen():
        ...
        try:
            ...
            if task_id:
                key = REDIS_TASK_STOP_KEY_FMT.format(task_id)
                try:
                    exists = await redis.exists(key)
                except Exception as e:
                    log.exception("Error checking stop key: %s", e)
                    exists = 0
...
async def stop_task(redis, task_id: str):
    ...
    if redis:
        ...
        # )
        key = REDIS_TASK_STOP_KEY_FMT.format(task_id)
        # Set stop flag with TTL to avoid זליגת flags
        await redis.set(key, "1", ex=STOP_KEY_TTL_SECONDS)
